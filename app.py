...
MODEL_PATHS = [
    "models/7B/llama-2-7b-chat.ggmlv3.q4_0.bin",
    "",
]

# create a streamlit dropdown menu to select the model
model_path = streamlit.sidebar.selectbox(
    "Select a model",
    MODEL_PATHS,
    index=0,
)

llm = load_llm_model_from_path(model_path)

# create a text box to enter the prompt
prompt = streamlit.text_area("Enter a prompt", value="Hello, how are you?")
# create a button to submit the prompt
submit = streamlit.button("Submit")

# if the button is clicked, generate a response
if submit:
    with streamlit.spinner("Generating response..."):
        ...
        streamlit.write(response["choices"][0]["text"])
